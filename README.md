...
```c
RANGE(u32) *array = RANGE_CREATE_STATIC(u32, 8u, { 5, 2, 1, 4, 3, });
...
All of the function's library that need to allocate dynamic memory must take a `struct allocator` as one of their parameters, and is discouraged to store it somewhere for later reuse, as it kinda breaks the whole principle behind it.
